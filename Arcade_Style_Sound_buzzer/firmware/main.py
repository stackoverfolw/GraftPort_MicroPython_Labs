...
from drivers.passive_buzzer_driver import Buzzer   # 蜂鸣器驱动
...
def button_handler(pin: Pin) -> None:
    ...
    """
    global hal, audio, BUzzer, sensor_task
...
# ------------------------------ 仅修改以下硬件初始化部分 ------------------------------
# 1. 初始化蜂鸣器扬声器（仅引脚初始化）
BUzzer_pin = board.get_dio_pins(1)[0]  # 获取引脚号
BUzzer = Buzzer(BUzzer_pin)  # 创建对象（按驱动要求传引脚）
...
# ------------------------------ 仅修改任务创建部分 ------------------------------
# 创建传感器任务实例（传入正确的硬件对象）
sensor_task_obj = SensorAudioTask(hall_sensor=hal, mic_sensor=audio, buzzer=BUzzer, enable_debug=True)
